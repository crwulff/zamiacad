 ...
 *
 */
public class TargetArrayCSelSymbol extends Symbol {

	public static final int PORT_DISTANCE = 40;
	private HashMap<RTLPort, Position> portPositions;

	private int width, height;
	private RTLTargetArrayCSel csel;
	
	public TargetArrayCSelSymbol(RTLTargetArrayCSel csel_) {
		csel = csel_;
	...
	public void paint(RTLView v_, GC gc, RTLModule module_, int xpos,
		...
		for (int i = 0; i < csel.getNumInputs(); i++) {
			...
			int offset = csel.getInputOffset(i);
		...
		//gc.drawRectangle(tX(v_, 0, xpos), tY(v_, 0, ypos), tW(v_, getWidth()), tH(v_, getHeight()));
