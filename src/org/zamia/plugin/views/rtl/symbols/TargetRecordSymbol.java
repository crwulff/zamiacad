 ...
 *
 */
public class TargetRecordSymbol extends Symbol {

	public static final int PORT_DISTANCE = 40;
	private HashMap<RTLPort, Position> portPositions;

	private int width, height;
	private RTLTargetRecord targetRecord;
	
	public TargetRecordSymbol(RTLTargetRecord csel_, RTLView control_) {
		targetRecord = csel_;
	...
	private void calcPortPositions(RTLView control_) {
		...
		int n = targetRecord.getNumInputs();
		
		height = Math.max(n,2) * PORT_DISTANCE + 6;
		
		int yi = (height - n * PORT_DISTANCE)/2+3 ; // current input port position
		for (int i = 0; i < targetRecord.getNumInputs(); i++) {
			RTLPort p = targetRecord.getInput(i);
			portPositions.put(p, new Position(0, yi));

			String l = targetRecord.getInputId(i);
		...
		portPositions.put(targetRecord.getZ(), new Position(width, height/2-PORT_DISTANCE/2));
	}
	...
	public void paint(RTLView v_, GC gc, RTLModule module_, int xpos,
		...
		for (int i = 0; i < targetRecord.getNumInputs(); i++) {
			...
			String id = targetRecord.getInputId(i);
